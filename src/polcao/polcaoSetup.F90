...
subroutine getArrAtomPairs(arrinfo, blcsinfo, atomPairs, atomTree, whichArr)
   ...
   ! Define passed parameters
   type(sArrayInfo), intent(inout) :: arrinfo
   type(BlacsInfo), intent(in) :: blcsinfo
   type(AtomPair),  pointer :: atomPairs
   ...
   do i=0,arrinfo%nrblocks-1
      do j=0,arrinfo%ncblocks-1
         ...
         do kpl=1,numKPoints 
            ...
            case(4)
               call h5dwrite_f(datasetToWrite_did(kpl, &
...
subroutine localToGlobalMap(a, b, glo, ghi, arrinfo, blcsinfo, extra)
   ...
   ! Define passed parameters
   type(BlacsInfo), intent(in) :: blcsinfo
   type(sArrayInfo), intent(in) :: arrinfo
